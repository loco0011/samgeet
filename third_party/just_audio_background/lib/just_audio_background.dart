...
class _PlayerAudioHandler extends BaseAudioHandler
  ...
  Future<void> skipToPrevious() async {
    // Samgeet: like the in-app button, restart the song unless it has only just begun.
    if (hasPrevious && currentPosition < const Duration(seconds: 4)) {
  ...
  /// Samgeet: swiping the app away from recent apps stops the music (just
  /// leaving the app, or locking the phone, keeps it playing).
  @override
  ...
  void _broadcastState() {
    // Samgeet: Previous is always shown (it restarts the song when there is
    // nothing before it) and there is no Stop button.
    final controls = [
